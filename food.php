...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark fixed-top">
	...
	{
	?>
	<a class="navbar-brand" style="color:#fff; text-decoration:none;"><i class="far fa-user"><?php if(isset($id)) { echo $vr['fld_name']; }?></i></a>
	<?php
	}
	?>
			 ...
			 <div class="tab-pane fade " id="status" role="tabpanel" aria-labelledby="status-tab">
				...
				if(mysqli_num_rows($orderquery))
				{
					while($orderrow=mysqli_fetch_array($orderquery))
					{
						...
						<?php 
						$qry = "SELECT fld_mobile, fld_address FROM tblcustomer WHERE fld_email ='$email'";
						$res=mysqli_query($con, $qry);
						if(mysqli_num_rows($res)){
						while ($result=mysqlI_fetch_array($res)) {
						?>
						<td><?php echo $result['fld_mobile']; ?><br>
							<?php echo $result['fld_address']; ?></td>
						<?php }
					}else{
						?>
						<td></td>
